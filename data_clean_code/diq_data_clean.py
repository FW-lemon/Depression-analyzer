...
def clean_diq_data(df):
    ...
    valid_ranges = {
        ...
        'DIQ060U': {1, 2},
        'DIQ070': {1, 2, 7, 9},
        # 早期或样本数据中的变量（假设范围，需确认早期文档）
        'DIQ220': {1, 2, 7, 9},  # 假设为二元或拒绝/不知道
        'DIQ190A': {1, 2, 7, 9},
        'DIQ190B': {1, 2, 7, 9},
        'DIQ190C': {1, 2, 7, 9},
        'DIQ200A': {1, 2, 7, 9},
        ...
        'DIQ230': {1, 2, 7, 9},
        'DIQ240': {1, 2, 7, 9},
        'DID250': set(range(1, 100)) | {666, 777, 999},  # 假设为数值
        'DID260': set(range(1, 100)) | {666, 777, 999},
        'DIQ260U': {1, 2, 7, 9},
        'DID270': set(range(1, 100)) | {666, 777, 999},
        'DIQ280': {1, 2, 7, 9},
        'DIQ290': {1, 2, 7, 9},
        'DIQ300S': set(range(0, 300)) | {6666, 7777, 9999},  # 样本数据中的占位符
    ...
    # 一致性检查
    # 1. 如果 DIQ010 ≠ 1，则 DID040、DIQ050、DID060、DIQ060U、DIQ070 应为缺失
    ...
    # 2. 如果 DIQ050 ≠ 1，则 DID060、DIQ060U 应为缺失
...
def load_data(file_path):
    # 读取CSV，将5.397605346934028e-79、9999.0、6666.0等占位符视为NaN
    df = pd.read_csv(file_path, na_values=[5.397605346934028e-79, '', np.nan, 9999.0, 6666.0, 7777.0])
    return df
